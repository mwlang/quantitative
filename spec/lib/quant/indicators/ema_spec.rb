...
RSpec.describe Quant::Indicators::Ema do
  ...
  context "EMA" do
    ...
    it { expect(subject.values.map{ |v| v.ema_half_period.round(3) }).to eq([3.0, 3.2, 3.787, 5.134]) }
  ...
  context "SS" do
    ...
    it { expect(subject.values.map{ |v| v.ss_half_period.round(3) }).to eq([3.0, 3.06, 3.242, 3.707]) }
  ...
  context "oscillators" do
    ...
    it { expect(subject.values.map{ |v| v.osc_half_period.round(3) }).to eq([0.0, -0.14, -0.545, -1.428]) }
